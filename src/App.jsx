...
function FarmApp() {
  ...
  const sideW = sideOpen ? 210 : 58;
  return (
    <div style={{ fontFamily: "'Inter','Segoe UI',sans-serif", background: C.bg, minHeight: "100vh", display: "flex", color: C.text }}>
      <div style={{ width: sideW, minHeight: "100vh", background: C.primary, display: "flex", flexDirection: "column", transition: "width .2s", overflow: "hidden", flexShrink: 0 }}>
        <div style={{ padding: "18px 14px 14px", borderBottom: "1px solid rgba(255,255,255,.15)", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {sideOpen && <div style={{ color: "#fff", fontWeight: 800, fontSize: 13, lineHeight: 1.2 }}>Fazenda<br /><span style={{ color: C.accent, fontSize: 11 }}>GOIABA</span></div>}
        </div>
        ...
        <button onClick={() => setSideOpen(o => !o)} style={{ margin: "0 8px 14px", padding: 8, border: "none", background: "rgba(255,255,255,.1)", borderRadius: 8, color: "#fff", cursor: "pointer" }}>{sideOpen ? "◀" : "▶"}</button>
      </div>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <div style={{ background: C.card, borderBottom: `1px solid ${C.border}`, padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <h1 style={{ margin: 0, fontSize: 18, color: C.text, fontWeight: 700 }}>{navItems.find(n => n.key === page)?.label}</h1>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <span style={{ fontSize: 12, color: saveState === "error" ? C.danger : saveState === "saved" ? C.primary : C.muted, fontWeight: saveState === "idle" ? 400 : 600, minWidth: 110, textAlign: "right" }}>
            ...
            <Btn variant="accent" size="sm" onClick={() => exportToExcel(data)}><Icon name="excel" size={16} color="#1A2E1A" /> Exportar Excel</Btn>
          </div>
        </div>
