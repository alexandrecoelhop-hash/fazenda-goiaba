...
import { listaSugestoes, VARIEDADES, VALVULAS } from "../lib/registros";
...
export default function Plots({ data, setData }) {
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}><h2 style={{ margin: 0, color: C.text }}>Válvulas</h2><Btn onClick={openNew}><Icon name="plus" size={16} color="#fff" /> Nova Válvula</Btn></div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>
        {data.plots.length === 0 && <p style={{ color: C.muted }}>Nenhuma válvula cadastrada. Cadastre a Válvula 1 e a Válvula 2.</p>}
        {data.plots.map(p => (
      ...
      {modal && <Modal title={modal === "edit" ? "Editar Válvula" : modal === "copy" ? "Copiar Válvula" : "Cadastrar Válvula"} onClose={() => setModal(null)}><div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}><Input label="Nome" value={form.name} onChange={v => setForm(f => ({ ...f, name: v }))} suggestions={VALVULAS} placeholder="Válvula 1" required /><Input label="Variedade" value={form.variety} onChange={v => setForm(f => ({ ...f, variety: v }))} suggestions={variedades} placeholder="Paluma, Pedro Sato..." /></div>
