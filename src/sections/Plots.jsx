...
export default function Plots({ data, setData }) {
  ...
  const [form, setForm] = useState({ name: "", variety: "", area: "", plantYear: "", plantCount: "", irrigation: "gotejamento", stage: "produção", notes: "" });
  ...
  const save = () => { setData(d => ({ ...d, plots: [{ ...form, id: uid() }, ...d.plots] })); setModal(false); };
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}><h2 style={{ margin: 0, color: C.text }}>Talhões / Blocos</h2><Btn onClick={() => { setForm({ name: "", variety: "", area: "", plantYear: "", plantCount: "", irrigation: "gotejamento", stage: "produção", notes: "" }); setModal(true); }}><Icon name="plus" size={16} color="#fff" /> Novo Talhão</Btn></div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>
        ...
        {data.plots.map(p => (
          <Card key={p.id} style={{ borderLeft: `4px solid ${C.primary}` }}>
            ...
            <div style={{ marginTop: 12 }}><Btn size="sm" variant="danger" onClick={() => setData(d => ({ ...d, plots: d.plots.filter(x => x.id !== p.id) }))}><Icon name="trash" size={14} color="#fff" /> Remover</Btn></div>
          </Card>
        ))}
      </div>
      {modal && <Modal title="Cadastrar Talhão" onClose={() => setModal(false)}><div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
