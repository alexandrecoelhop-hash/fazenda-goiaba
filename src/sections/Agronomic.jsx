...
export default function Agronomic({ data, setData }) {
  ...
  const talhoes = listaSugestoes([], data.plots.map(p => p.name), data.agronomicEvents.map(e => e.talhao), data.applications.map(a => a.talhao));
  ...
  return (
    <div>
      ...
      {modal && <Modal title={modal === "edit" ? "Editar — Evento de Manejo" : modal === "copy" ? "Copiar — Evento de Manejo" : "Evento de Manejo"} onClose={() => setModal(null)}><div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}><Input label="Talhão" value={form.talhao} onChange={v => setForm(f => ({ ...f, talhao: v }))} suggestions={talhoes} /><Input label="Área (ha)" type="number" value={form.area} onChange={v => setForm(f => ({ ...f, area: v }))} /></div>
