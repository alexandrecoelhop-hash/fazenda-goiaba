...
import { Card, Btn, Badge, Icon, Input, Modal, Table } from "../ui";

// ─── APLICAÇÕES (caderno de campo fitossanitário) ────────────────────────────
export default function Applications({ data, setData }) {
  const [modal, setModal] = useState(false);
  const [form, setForm] = useState({ date: today(), talhao: "", target: "", product: "", active: "", dose: "", volume: "", carencia: "", reentry: "", applicator: "", notes: "" });
  const save = () => { setData(d => ({ ...d, applications: [{ ...form, id: uid() }, ...d.applications] })); setModal(false); };
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        ...
        <Btn onClick={() => { setForm({ date: today(), talhao: "", target: "", product: "", active: "", dose: "", volume: "", carencia: "", reentry: "", applicator: "", notes: "" }); setModal(true); }}><Icon name="plus" size={16} color="#fff" /> Nova Aplicação</Btn>
      </div>
      <Card>
        <Table cols={[
          ...
          { key: "del", label: "", render: r => <Btn size="sm" variant="danger" onClick={() => setData(d => ({ ...d, applications: d.applications.filter(x => x.id !== r.id) }))}><Icon name="trash" size={14} color="#fff" /></Btn> },
        ]} rows={data.applications} />
      </Card>
      {modal && (
        <Modal title="Registro de Aplicação" onClose={() => setModal(false)} wide>
